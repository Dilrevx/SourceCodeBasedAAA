...
class LibRadarLite(object):
    ...
    def __close__(self):
        # Close dex file
        for dex_object in self.dex_objects:
            dex_object.close_DexFile()
...
#                os.removedirs(os.path.dirname(dex_name))
                try:
                    print(os.path.dirname(dex_name))
    ...
    def decode(self):
        ...
        # Save all the .dex and decoded resource files contained in the apk into the Decompiled directory and save
        # the .dex file names into the dex_names array.
        self.dec_path = SCRIPT_PATH.replace('\\', '/') + "/Data/Decompiled/%s" % self.hex_sha256

        #type_name = self.apk_path.split("/")[-4]
        market_name = self.apk_path.split("/")[-3]
        writer_name = self.apk_path.split("/")[-2]
        ...
        for file_name in os.listdir(self.dec_path):
            if file_name.endswith('.dex'):
                self.dex_names.append(self.dec_path + '/' + file_name)
...
#        self.unzip()
        # edit by zcp - Step 1: Decomplie APK file, include the resource file and dex files.
        self.decode()
        return
...
#    iron_apk_path = u'e:/Apks/com.ea.game.pvz2_row.apk'
    iron_apk_path = u'e:/Apks/fq.router2-2.apk'
